...
#### 构造函数
```cpp
MovePicker(pos, mem, info, history, tt_move, ply, prev_move, prev2_move, depth, quiet_control)
```
...
```cpp
score = quiet_ordering_score_fast(pos, move, prev_move, prev2_move)
```
...
Five terms (see History.md): quiet history, countermove bonus, continuation 1, continuation 2, pawn history.
